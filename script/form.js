...
cardButton.forEach((button) => {
  button.addEventListener("click", (e) => {
    ...
    wrapper.style =
      "position:absolute; top:0;left: 0;width:100vw; height:100vh;background-color:rgba(0,0,0,0.2);z-index:200";
...
function closeModal() {
  ...
  formModal.classList.replace("show", "hidden");
}
...
function createOptions(arr) {
  const regexp = /\s/g;
  let optionArr = arr.map(
    (item) =>
      `<label class="option__label option"><input type="checkbox" class="option__checkbox option" name=${item.replace(
        regexp,
        ""
      )}>${item}</label>`
  );
...
function filterOptions(e) {
  ...
  let filteredArr = applications.filter((string) =>
    string.toLowerCase().includes(e.target.value.toLowerCase())
  );
  filteredArr.length === 0
    ? (optionList.style.display = "none")
    : createOptions(filteredArr);
}
  ...
  for (let i = 1; i <= 10; i++) {
    amountListHTML += `
            <label class="value form__value amt">${i}
                <input type="radio" class="value__input amt" name="value" value="${i}" ${
      i === 1 ? "checked" : ""
    }>
            </label>
        `;
      ...
      (block.onclick = function () {
        if (isAmountShowing) {
          ...
          values.forEach(
            (value) =>
              (value.onclick = (e) => {
                e.target.closest(".amount__value").childNodes[0].innerText =
                  e.target.value;
                changePrice(
                  e.target.value,
                  e.target.closest(".summary__elem").childNodes[3]
                );
                e.target.closest(".amount__content").remove();
                isAmountShowing = true;
                costs.forEach((cost) =>
                  costArr.push(Number.parseInt(cost.innerText))
                );
    ...
    (button) =>
      (button.onclick = (e) => {
        ...
        options.querySelector(
          `[name=${e.target.closest(".summary__elem").getAttribute("name")}]`
        ).checked = false;
      })
  );
}
...
function checkboxesChange(e) {
  ...
  e.target.checked
    ? createSummaryElem(e.target)
    : deleteSummaryElem(e.target.name);
}
...
form.onsubmit = (e) => {
  ...
  thankYou.innerHTML = `
        ...
        <a href="#" class="button end__button" onClick="closeModal()">Вернуться на главную</a>
    `;
  document.querySelector(".modal-form").append(thankYou);
};
